...
const TopBar = () => {
  return (
    <div className="fixed top-0 left-0 right-0 h-16 bg-gray-900/80 backdrop-blur-sm flex items-center px-6 z-50 border-b border-orange-800">
      <h1 className="text-2xl font-michroma text-orange-400 tracking-tight">Scorch Overgrowth</h1>
    </div>
  );
};
...
export default function Home() {
    ...
    try {
      ...
      while (true) {
        ...
        for (const line of lines) {
          try {
            const parsed = JSON.parse(line);    // Create a new chat section with loading states
  ...
  return (
    <div className="min-h-screen bg-gray-900">
      <TopBar />
      <div className="pt-14 pb-24 bg-gradient-to-b from-gray-900 via-gray-800/30 to-gray-900"> {/* Add padding top to account for fixed header */}
            ...
            {!hasSubmitted ? (
                ...
                <form onSubmit={handleSubmit} className="w-full max-w-[704px] mx-4">
                  <div className="relative">
                    <input
                      ...
                      className="
                        w-full px-6 py-3 bg-gray-800/40 transition-all duration-250 ease-in-out text-sm text-orange-300
                        border border-orange-700 rounded-full hover:px-0 hover:border-transparent hover:border-b-orange-500
                        hover:rounded-none hover:bg-[position:100%_center] focus:outline-none focus:px-0 
                        focus:border-transparent focus:border-b-orange-500 focus:rounded-none focus:bg-[position:100%_center]
                        placeholder:text-orange-500/70 placeholder:uppercase placeholder:tracking-[1.5px] pr-20
                  "
                      onKeyDown={(e) => {
                    ...
                    />
                    <div className="absolute right-3 bottom-3 top-1/2 -translate-y-1/2 flex items-center gap-2">
                      <button
                        type="submit"
                        disabled={isLoading}
              ...
              >
                {chatSections.map((section, index) => (
                  <div key={index} className="pt-8 border-b border-white-200 last:border-0">
                    {/* Query */}
                    <div className="mb-8">
                      ...
                      <div className="mb-12">
                        ...
                        <div className="flex gap-3 overflow-x-auto pb-4 -mx-4 px-4">
                          {section.searchResults.map((result, idx) => (
                            <div
                              key={idx}
                              className="flex-shrink-0 w-[300px] bg-gray-800/40 border border-orange-800 rounded-xl overflow-hidden backdrop-blur-sm"
                            >
                              <div className="h-40 bg-gray-700/30 overflow-hidden relative">
                                {result.image ? (
                                  <>
                              ...
                              <div className="p-4">
                                <a
                                  ...
                                  className="text-orange-400 hover:text-orange-300 hover:underline block mb-2 font-medium line-clamp-2"
                                >
                                  {result.title}
                                </a>
                                <p className="text-sm text-orange-300/80 line-clamp-3">{result.content}</p>
                              </div>
                            </div>
                          ))}
                        </div>
                      ...
                      <div className="mb-12">
                        <div className="flex justify-between items-center mb-4">
                          <div className="flex items-center gap-2">
                            ...
                            <h3 className="text-sm font-semibold text-white">Thinking Process:</h3>
                          </div>
                          <div className="flex items-center gap-2">
                            <button
                              onClick={() => {
                        ...
                        <motion.div
                          className="pl-4 border-l-2 border-orange-600"
                          initial={false}
                          animate={{
                          ...
                          transition={{ duration: 0.3 }}
                        >
                          <div className="text-sm text-orange-300/90 leading-relaxed overflow-hidden">
                            {section.reasoning}
                          </div>
                        </motion.div>
                      </div>
                    ...
                    {section.response && (
                      <div className="mt-12 mb-16">
                        <div className="prose prose-invert max-w-none space-y-4 [&>ul]:list-disc [&>ul]:pl-6 [&>ol]:list-decimal [&>ol]:pl-6
                    [&_h1]:text-orange-400 [&_h2]:text-orange-400 [&_h3]:text-orange-400 [&_a]:text-orange-400 
                    [&_a:hover]:text-orange-300 [&_pre]:bg-gray-800/40 [&_pre]:p-4 [&_pre]:rounded-lg">
                          <ReactMarkdown
                            ...
                            components={{
                              ...
                              th: ({ node, ...props }) => (
                                <th
                                  className="py-3 px-4 font-medium text-md text-white border-b border-gray-200"
                                  {...props}
                                />
                              ),
                              td: ({ node, ...props }) => {
                                ...
                                if (content.match(/\[.*?\]\(.*?\)/)) {
                                  return (
                                    <td className="py-3 px-4 text-md text-gray-500">
                                      <ReactMarkdown
                                        components={{
                                          a: ({ node, ...linkProps }) => (
                                            <a {...linkProps} className="text-blue-600 hover:text-blue hover:underline" target="_blank" rel="noopener noreferrer" />
                                          )
                                        }}
                                      >
                                        {content}
                                        ...
                                        <thead className="bg-gray-50">
                                          <tr>
                                            {headers.map((header, i) => (
                                              <th key={i} className="py-3 px-4 font-medium text-md text-white border-b border-gray-200">
                                                {header}
                                              </th>
                                            ))}
                                          </tr>
                                ...
                                if (sourceMatch) {
                                  ...
                                  return (
                                    <span className="inline-flex items-center group relative">
                                      <a {...props} className="inline-flex items-center text-blue hover:text-blue-800">
          ...
          <form onSubmit={handleSubmit} className="w-full max-w-[704px] mx-4">
            <div className="relative">
              <input
                ...
                className="w-full px-6 py-3 bg-gray-800/40 transition-all duration-250 ease-in-out text-sm text-orange-300
      border border-orange-700 rounded-full hover:px-0 hover:border-transparent hover:border-b-orange-500
      hover:rounded-none hover:bg-[position:100%_center] focus:outline-none focus:px-0 
      focus:border-transparent focus:border-b-orange-500 focus:rounded-none focus:bg-[position:100%_center]
      placeholder:text-orange-500/70 placeholder:uppercase placeholder:tracking-[1.5px] pr-20" // Added padding-right
                onKeyDown={(e) => {
                  if (e.key === 'Enter' && !e.shiftKey) {
                    e.preventDefault();
                    handleSubmit(e);
              ...
              />
              <div className="absolute right-4 top-1/2 -translate-y-1/2"> {/* Adjusted positioning */}
                <button
                  type="submit"
                  disabled={isLoading}
      ...
      {showTavilyModal && (
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-lg p-6 max-w-2xl w-full max-h-[80vh] overflow-y-auto">
            <div className="bg-gray-800 rounded-lg p-6 max-w-2xl w-full max-h-[80vh] overflow-y-auto border border-orange-800">
              <h3 className="text-lg font-semibold text-orange-400">Full Tavily Response</h3>
            ...
            <pre className="whitespace-pre-wrap text-sm text-orange-300/90 font-mono">
